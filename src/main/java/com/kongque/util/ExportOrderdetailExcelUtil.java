...
/**
 * @author cary time:2017年5月17日
 */
public class ExportOrderdetailExcelUtil {
	// 格式化时间
	 ...
	 * @param dataSet
	 *            要被导出的数据集
	 * @param excludedFieldes
	 *            需要从被导出数据集中剔除的属性字段名集合
	 * @return exportedModelList 将被导出成Excel数据的最终版本数据集，以List&lt;Map&lt;String,
	 *         Object&gt;&gt;类型返回
		...
		for (T data : dataSet) {
			...
			for (Field field : fields) {
				if (excludedFieldes != null && !excludedFieldes.contains(field.getName())) {
					...
					/*
					 * case "voucherType": if(field.get(data) != null){
					 * switch((int)field.get(data)){//[2017-08-01] case
					 * 1:exportedModel.put("voucherType", "微调单");break; case
					 * 2:exportedModel.put("voucherType", "订单");break;
					 * default:exportedModel.put("voucherType", ""); } }break;
					 */
	 ...
	 * @throws IOException
	 */
	public static <T> void exportExcel(int colSplit, int rowSplit, int dataBeginIndex, String sheetName,
			String[] headers, Collection<T> dataset, OutputStream out) throws IOException {
	 ...
	 *            表格属性列名数组
	 * @param dataset
	 *            1.需要显示的数据集合,集合中一定要放置符合javabean风格的类的对象。此方法支持的
	...
	public static <T> void exportExcel(int colSplit, int rowSplit, int dataBeginIndex, String title, String[] headers,
				...
				if (t instanceof Class) {
					...
					for (int i = 0; i < fields.length; i++) {
						...
						if (value instanceof byte[]) {
							...
							HSSFClientAnchor anchor = new HSSFClientAnchor(0, 0, 1023, 600, (short) i, index,
									(short) (i + 1), index + 1);
							anchor.setAnchorType(2);
							HSSFPicture pic = patriarch.createPicture(anchor,
									workbook.addPicture(bsValue, Workbook.PICTURE_TYPE_JPEG));
							 ...
							 * CommonsMethod.getNowCorrect2SecondWithEnglishWord();
							 */
							textValue = pattern == null ? new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(value)
					...
					while (keys.hasNext()) {
						...
						if (keyValue instanceof byte[]) {
							...
							HSSFClientAnchor anchor = new HSSFClientAnchor(0, 0, 1023, 255, (short) i, index,
									(short) (i + 1), index + 1);
							anchor.setAnchorType(2);
						...
						if ("num".equals(keyname)) {
							cell.setCellValue(Double.parseDouble(textValue));
							sum += Integer.parseInt(textValue);
						}
					...
					for (int i = 0; i < fields.length; i++) {
						...
						if (value == null) {
							...
							// 其它数据类型都当作字符串简单处理
							// modified by fengfeng.yu 20161115,
							// 如果value为null的话，空字符串无法进行处理
							textValue = value.toString();
						}
